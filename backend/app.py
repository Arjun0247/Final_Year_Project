...
try:
    ...
    print('class_names.json not found — using default class order')
    CLASSES = ['A+', 'A-', 'AB+', 'AB-', 'B+', 'B-', 'O+', 'O-']

# ─── Model ────────────────────────────────────────────────
MODEL_PATH = 'scanner_weights.pkl'
GDRIVE_FILE_ID = "1vBQQ9I-HRxxx8oAyzWdpG7eV1IEkpCbh" # <-- replace this
...
try:
    if os.path.exists(MODEL_PATH):
        ...
        print("Weights file not found. Using mock predictions.")
except Exception as e:
...
def predict():
    ...
    try:
        ...
        if model:
            ...
            print('Warning: Using mock predictions (model not loaded)')
